import {
  ...
  const craneRegex = new RegExp(
    /^[a-zA-Z]* {1}(\d*) {1}[a-zA-Z]* {1}(\d*) {1}[a-zA-Z]* {1}(\d*)/gm
  );
  ...
  const simulateCraneMoves = () => {
    parsedCraneData.forEach((instruction) => {
      const { amount, origin, destination } = instruction;
        ...
        let to = parsedCrateData[destination - 1];
        const boxes = from.splice(0, amount);
        const newStack = boxes.concat(to);

        if (boxes) parsedCrateData[destination - 1] = newStack;
    });
